...
import 'package:pictiknew/Screen/SelectedAlbum.dart';
import 'package:progress_dialog/progress_dialog.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../ImageView.dart';
import 'CollageGallaryComponent.dart';

class CollageGallaryScreen extends StatefulWidget {
  String albumId, albumName, totalImg, image;

  CollageGallaryScreen(
      {this.albumId, this.albumName, this.totalImg, this.image});
  @override
  _CollageGallaryScreenState createState() => _CollageGallaryScreenState();
}

class _CollageGallaryScreenState extends State<CollageGallaryScreen> {
  ...
  /* downloadAll() async {
    ...
    for (int i = 0; i < selectedPhone.length; i++) {
      String path =
          "${ImgUrl}${selectedPhone[i]["ImageUrl"].toString().replaceAll(" ", "%20")}";
  ...
  downloadAll() async {
    ...
    for (int i = 0; i < selectedPhoto.length; i++) {
      String path =
          "${ImgUrl}${selectedPhoto[i]["Photo"].toString().replaceAll(" ", "%20")}";
      Platform.isIOS
          ? await GallerySaver.saveImage(path).then((bool success) {
              print("Success = ${success}");
              Fluttertoast.showToast(
            ...
            new FlatButton(
              ...
              onPressed: () {
                if (edtPIN.text == SelectedPin) {
                  ...
                  Fluttertoast.showToast(
                      msg: "Enter Valid PIN...",
                      textColor: appPrimaryMaterialColorPink[700],
                ...
                Expanded(
                  child: albumData.length != 0 && albumData != null
                      ...
                      ? StaggeredGridView.countBuilder(
                          ...
                          itemCount: albumData.length,
                          itemBuilder: (BuildContext context, int index) {
                            return CollageGallaryComponent(
