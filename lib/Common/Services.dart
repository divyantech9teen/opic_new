...
import 'package:pictiknew/Common/ClassList.dart';
...
class Services {
  ...
  static Future<List> userloginwithusername(String username,String password) async {
    String url = API_URL + 'CustomerLogin?username=$username&password=$password';
  ...
  static Future<List<MemberClass1>> CardIdNight(String mobileno) async {
    String url =
        "http://digitalcard.co.in/DigitalcardService.asmx/"+ 'Member_login?type=mobilelogin&mobileno=$mobileno';
  ...
  static Future<List> GetCardIdLogin(String type,String MobileNo) async {
    String url = "http://pmc.studyfield.com/Service.asmx/" + 'Login?type=$type&mobile=$MobileNo';
  ...
  static Future<List<MemberClass1>> UpdateCardId(String type,String cardid,String userid) async {
    String url = "http://pmc.studyfield.com/Service.asmx/" + 'UpdateDigitalCardId?type=$type&cardid=$cardid&userid=$userid';
  ...
  static Future<List<MemberClass1>> MemberLogin1(String mobileno) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final jsonResponse = json.decode(response.body);
        MemberDataClass1 memberDataClass = new MemberDataClass1.fromJson(jsonResponse);
        if (memberDataClass.ERROR_STATUS == false) {
          list = memberDataClass.Data;
        }
        else
  ...
  static Future<SaveDataClass1> MemberSignUp(data) async {
    ...
    try {
      if (response.statusCode == 200) {
        SaveDataClass1 data;
        final jsonResponse = json.decode(response.body);
        SaveDataClass1 saveDataClass = new SaveDataClass1.fromJson(jsonResponse);
  ...
  static Future<List> GetPhotogrpaherOffers(String photographerid) async {
    String url = API_URL + 'GetPhotographerOffers?type=offers&photographerid=$photographerid';
  ...
  static Future<Map> GetUserData(String parentid,String name,String mobile,String email,String studioid,String fcmtoken) async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
    String StudioId = preferences.getString(Session.StudioId);
    String url = API_URL + 'UpdateUserData?parentid=$parentid&name=$name&mobile=$mobile&email=$email&studioid=$studioid&fcmtoken=$fcmtoken';
